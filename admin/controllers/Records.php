...
class Records extends Admin_controller {
	...
	protected $_records_validation_rules = array(
    ...
    function index()
    {
        ...
        $str = '<a href="'.site_url('records/add_edit_records/{id}').'" class="table-link">
                    <span class="fa-stack">
        ...
        $listing = $this->listing->get_listings('records_model', 'listing');
	...
	public function add_edit_records($edit_id = "")
    {            
		//print_r($_POST);exit;
		...
		$this->data['get_menu'] = $this->records_model->get_menu_osha("users",array("role" => 2));
		...
		if(is_logged_in()) {
			...
			$this->form_validation->set_rules($this->_records_validation_rules);
        ...
        if($this->form_validation->run())
        {
			...
			if(empty($edit_id)){
                     ...
                     $social_data = $this->records_model->insert("records",$ins_data);
                    // $this->service_message->set_flash_message('record_insert_success');
		      }
              else 
              {  echo 
                    $social_data = $this->records_model->update("records",$ins_data,array("id" => $edit_id));
			 ...
			 if($edit_id) {
                $edit_data = $this->records_model->get_slideimage_detail("records",array("id" => $edit_id));
               
                if(!isset($edit_data[0])) {
                   // $this->service_message->set_flash_message('record_not_found_error');
                    redirect("records");   
                }
                $this->data['title']          = "EDIT RECORDS";
	...
	function do_upload()
	{
		...
		$config['upload_path'] = '../assets/images/frontend/records';
	...
	function records_delete()
    {
       
        $id = ($_POST['id'])?$_POST['id']:"";
        if(!empty($id)) {
            
            $this->db->query('delete from records where id in ('.$id.')');
