...
class Documents extends Admin_controller {
    ...
    function index()
    {
        ...
        $listing = $this->listing->get_listings('logs_model', 'listing');
    ...
    {            
		...
		$this->data['get_menu'] = $this->logs_model->get_menu_osha("users",array("role" => 2));
		...
		if(is_logged_in()) {
			...
			$this->form_validation->set_rules($this->_logs_validation_rules);
	...
	function do_upload()
	{
		...
		$config['upload_path'] = '../assets/images/frontend/logs';
	...
	function documents_delete()
    {
        ...
        if(!empty($id)) {
            ...
            $this->db->query('delete from logs where id in ('.$id.')');
