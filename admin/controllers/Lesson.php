...
class Lesson extends Admin_controller {
    ...
    { 
       ...
       //$this->load_settings_data();
        ...
        $str = '<a href="'.site_url('lesson/add_lesson/{id}').'" class="table-link">
                    <span class="fa-stack">
        ...
        $listing = $this->listing->get_listings('lesson_model', 'listing');
        ...
        if(is_logged_in())        
        	$this->layout->view("lesson/lesson_list");
        else
        	redirect("login");
	...
	public function add_lesson($edit_id = "")
    { 
		...
		$this->data['get_menu'] = $this->lesson_model->get_menu("users",array("role" => 2));
        ...
        { 
			...
			$update_data = $this->lesson_model->insert_lesson("lession",$ins_data);
			$this->data["lesson_id"] = $update_data;
           $this->session->set_flashdata("lesson_succ","Lesson added successfully",TRUE);
		}else {
			
			$update_data = $this->lesson_model->update("lession",$ins_data,array("id" => $edit_id));
		...
		//redirect("lession");  
			 ...
			 if($edit_id) {
                $edit_data = $this->lesson_model->get_lession_data("lession",array("id" => $edit_id));
            ...
            if(!empty($lesson_id)){
				...
				$this->data['attachment_list'] = $this->lesson_model->get_lession_attachment_list($lesson_id);  
				
			}else if(!empty($edit_id)){
				
				$this->data['attachment_list'] = $this->lesson_model->get_lession_attachment_list($edit_id);  
			}
			
			$this->data['attachment_display'] = $this->load->view('lesson/attachment_display',$this->data,TRUE);
		
		 
		$this->layout->view('lesson/add');
	...
	*/
    ...
    function lesson_content()
    {
		...
		if(is_logged_in()) {
            ...
            $info = $this->lesson_model->get_info("lesson_content");
        ...
        { 
			...
			$social_data = $this->lesson_model->update("lesson_content",$ins_data,array("id" => $edit_id));
		...
		$this->layout->view('lesson/lesson_content');
