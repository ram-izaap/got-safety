...
class AuthorizeCimLib
{
	...
	function __construct()
	{
		...
		$this->_CI->config->load('authorize_net');
		
		if($this->_CI->config->item('api_login_id') && 
				$this->_CI->config->item('api_transaction_key') &&
				$this->_CI->config->item('arb_api_url'))
		{
			$this->initialize($this->_CI->config->item('api_login_id'), 
												$this->_CI->config->item('api_transaction_key'), 
												$this->_CI->config->item('arb_api_url'));
		}
